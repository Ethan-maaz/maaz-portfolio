...
async function main() {
  await mongoose.connect('mongodb+srv://maazajaz:7565maaz@cluster0.kflqr.mongodb.net/blogDB');
}
...
app.post("/edit/:name",function(req,res){
  ...
  Post.replaceOne({title: req.params.name},{title: lodash.capitalize(req.body.title), content: new_content},function(err){
    if(err)
    console.log(err);
    else
    res.redirect("/home");
